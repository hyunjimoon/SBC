...
sc_opt <- self_calib_adaptive(calib_generator, backend_binom_opt, updator, "eta", lambda_mu, lambda_sigma, nsims, gamma, tol, fixed_args = fixed_args_binom)

datasets_new <- generate_datasets(SBC_generator_function(generator_binom, sc_opt$mu, sc_opt$sigma, fixed_args_binom), n_datasets = fixed_args_binom$nsims)
result_binom_opt_new <- compute_results(datasets_new, backend_binom_opt, thin_ranks = 1)
...
# result in Error in chol.default(-H) : the leading minor of order 13 is not positive definite

backend_gr_hmc <- SBC_backend_cmdstan_sample(cmdstan_mod_gr, chains = 4, iter_sampling = ndraws / 4) # thin = 10
sc_opt <- self_calib_adaptive(calib_generator, backend_gr_hmc, updator, "eta", lambda_mu, lambda_sigma, nsims, gamma, tol, fixed_args = fixed_args_gamma)
...
lambda_mu_gmm <- rvar(array(rep(10, 10,  each = nsims), dim = c(nsims, nsims)))
