...
We introduce a self-calibration algorithm. First, dap operator $\hat{g}(f(\Phi X, U), \Phi X)$ is inspected to verify its sublinearity and monotonicity. Then, based on this hyperparmeter self-calibration algorithm is suggested. Final improvement of calibration is compared with SBC rank plot.
...
prior_dap <- list(mu = c(), logsigma = c(), dap_mu = c(), dap_logsigma = c())
for (mu in seq(-4, 4, by = 2)){
  for(logsigma in seq(-4, 4, by = 2)){
    prior_dap$mu <- c(prior_dap$mu, mu)
    prior_dap$logsigma <- c(prior_dap$logsigma, logsigma)
    dap <- calculate_dap(mu, exp(logsigma), generator_binom, backend_binom_opt, fixed_args_binom)
    prior_dap$dap_mu <- c(prior_dap$dap_mu, dap$mu)
    prior_dap$dap_logsigma <- c(prior_dap$dap_logsigma, log(dap$sigma))
  }
}
...
for (mu in seq(-5, 5, by = 2)){
  for(sigma in seq(1, 9, by = 3)){
...
ggplot(prior_dap_gr) + geom_density_2d(aes(x=mu, y = sigma)) + geom_density_2d(aes(x=dap_mu, y = dap_sigma), color = "red")
```
