...
# Experiment 2.
Quantile update algorithm is used with quanitle regression loss function. With larger `nsims`, it reaches well-calibrated region with lower `SBC_iter`
...
It has the problem of preferring smaller
```{r, warning=FALSE, error=FALSE}
#model = stan_model("./models/binom-laplace.stan")
SBC_iter <- 91
# prior hyperparameters
...
for (j in 1:SBC_iter){
  ...
  #}
  # update hyperparameters depending on inference algorithm
  mu_dist <- posterior::draws_of(SBC::update_quantile_approximation(mu_dist, mu_hat_lst, nsims, -1, 1000, 1e-5))[1, ]
