...
# Experiment 3
Using gradient update for convergence
``` {r, warning=FALSE, error=FALSE}
model <- stan_model("models/binom-laplace.stan")
get_posterior <- function(eta, mu, sigma) {
...
# results from solving (x-y)^2 after x 
# fix points are all x = y
quad_recursion <- function(x, y) {
  (y^2 + x^2) / (2 * y)
}
...
for (j in 1:niter) {
  ...
  mu_new <- quad_recursion(mu_est, mu[j])
  mu[j+1] <- mu_new
  message("mu : ", mu[j], " mu_est : ", mu_est, " mu_new : ", mu_new)
  
  sigma_est <- sd(post_draws_eta)
  sigma_new <- quad_recursion(sigma_est, sigma[j])
  sigma[j+1] <- sigma_new
