...
# This vignette will always use cmdstanr as variational rstan backend
# is currently not implemented.
# It may however use either rstan or cmdstanr for the optimizing backend
library(cmdstanr)
if(!use_cmdstanr) {
  library(rstan)
...
if(use_cmdstanr) {
  ...
  cache_dir <- "./_approximate_computation_rstan_SBC_cache"
}
```
...
```{r}
model_poisson <- cmdstan_model("stan/poisson.stan", compile_model_methods = TRUE, compile_hessian_method = TRUE)
...
```{r}
if(use_cmdstanr) {
  backend_poisson_optimizing <- SBC_backend_cmdstan_optimize(model_poisson)  
} else {
...
model_HMM <- cmdstan_model("stan/hmm_poisson.stan", compile_model_methods = TRUE, compile_hessian_method = TRUE)
