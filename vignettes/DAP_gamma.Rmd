...
mu_vals <- seq(10, 15, length.out = 5)
var_vals <- seq(1, 5,length.out = 5)
results <- rep(NA, length(mu_vals))
dap_mu <- rep(NA, length(mu_vals))
# convexity of |x-T(x)| with different norms
convex <-array(rep(NA, length(mu_vals) * length(var_vals)), dim = c(length(mu_vals), length(var_vals)))
convex_v <-array(rep(NA, length(mu_vals) * length(var_vals)), dim = c(length(mu_vals), length(var_vals)))
...
contraction_v <- array(rep(NA, length(mu_vals) * length(var_vals)), dim = c(length(mu_vals), length(var_vals)))
colnames(convex) <- var_vals
rownames(convex) <- mu_vals
colnames(convex_v) <- var_vals
rownames(convex_v) <- mu_vals
colnames(contraction) <- var_vals
rownames(contraction) <- mu_vals
colnames(contraction_v) <- var_vals
rownames(contraction_v) <- mu_vals
...
for(i in 1:length(mu_vals)){
  for(j in 1:length(var_vals)){
    ...
    convex[i,j] <- (mu_vals[[i]] - dap$mu)^2 + (var_vals[[j]] - dap$var)^2
    convex_v[i,j] <- (mu_vals[[i]] - dap$mu)^2 + (var_vals[[j]]^2 - dap$var)^2
    
    contraction[i,j] <- (mu_vals[[i]] - dap$mu)^2 + (var_vals[[j]] - dap$var)^2 - ((dap$mu- dapdap$mu)^2 + (dap$var - dapdap$var)^2)
    
    contraction_v[i,j] <- (mu_vals[[i]] - dap$mu)^2 + (var_vals[[j]]^2 - dap$var^2)^2 - ((dap$mu- dapdap$mu)^2 + (dap$var^2 - dapdap$var^2)^2)
  }
}

persp3D(x=mu_vals, y=var_vals, z = convex, theta=75, phi=0, xlab="mu", ylab="sigma", zlab="convex", ticks=5, ticktype="detailed")
persp3D(x=mu_vals, y=var_vals, z = convex_v, theta=75, phi=0, xlab="mu", ylab="sigma", zlab="convex_v", ticks=5, ticktype="detailed")
...
persp3D(x=prior_dap$dap_mu, y=var_vals, z = convex_v, theta=30, phi=50, xlab="mu", ylab="var", zlab="convex")
```
