...
SBC_fit_to_bridge_sampler.SBC_backend_beta_binom2 <- function(backend, fit, generated, ...) {
  log_posterior_func <- function(draws.row, data) {
    if(backend$single_theta) {
      dbinom(sum(generated$y), size = 2 * generated$N, prob = draws.row["theta"])
    } else {
      dbinom(generated$y[1], size = generated$N, prob = draws.row["theta"]) + 
        dbinom(generated$y[2], size = generated$N, prob = draws.row["theta2"])
    }
  }
...
```{r}
res_custom2 <- compute_SBC(ds_custom2, backend_bs_custom2, globals = beta_binom2_globals, keep_fits = FALSE)
