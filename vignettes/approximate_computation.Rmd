...
S = 100
N = 50
hyperparam = list(prior_width = 1, N = N)
M = 1000
thin_ranks_s = 10
chains = 4
thin_ranks_v = 1
...
```{r warning=FALSE}
delivDir = set_get_Dir("two_normal")$delivDir
# target calibration
tv = c("loc", "scale")
S = 10 #1000
N = 10
...
backend_vi_mf <- SBC_backend_cmdstan_variational(mod_two_norm, output_samples = M * thin_ranks_v)
backend_vi_full <- SBC_backend_cmdstan_variational(mod_two_norm, output_samples = M * thin_ranks_v, algorithm = "fullrank")
result_0_mf <- compute_results(datasets, backend_vi_mf, thin_ranks_v) 
result_0_full <- compute_results(datasets, backend_vi_full, thin_ranks_v) 
plot_rank_hist(result_0_mf)
plot_rank_hist(result_0_full)
param_0 <- as_draws_rvars(subset_draws(datasets$parameters, variable = tv))
param_1 <- prop_param(param_0, result_0_full, thin_ranks_v, 0, delivDir)
#target calibration
evolve_df <- list()
for (v in tv){
 evolve_df[[v]] <- list(median = rep(NA,1), sd = rep(NA,1), scm = NA, scm_init = NA) 
}
param_sc_mf <- self_calib(generator, hyperparam, param_1, predictor, backend_vi_mf, tv, thin_ranks_v, 0, evolve_df, delivDir)
param_sc_full <- self_calib(generator, hyperparam, param_1, predictor, backend_vi_full, tv, thin_ranks_v, 0, evolve_df, delivDir)
...
result_sc <- compute_results(datasets_sc, backend_vi_full, thin_ranks_v) 
# before after compare with ecdf and rank summary
# graphical inspection
result_0 = result_0_full
g <- plot_rank_hist(result_0)
g_sc <- plot_rank_hist(result_sc)
# numeric inspection
next_param_0 <- prop_param(param_0, result_0, tv, cnt = 0, thin =1, delivDir) 
next_param_sc <- prop_param(param_sc, result_sc, tv, cnt = 0, thin =1, delivDir)
print(set2set(param_0, next_param_0, "loc")$CJS)
print(set2set(param_0, next_param_0, "scale")$CJS)
