...
# options(brms.backend = "cmdstanr") # Uncomment to use cmdstanr instead
...
The fits are very fast and we fit just a few, so we force a minimum chunk size to reduce overhead of
paralellization and decrease computation time.

```{r}
chunk_size <- max(5, default_chunk_size(length(datasets)))
results <- compute_results(datasets, backend, thin_ranks = 10, chunk_size = chunk_size)
```
