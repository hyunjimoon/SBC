...
```{r results_func2_meanfield_plots, eval=use_cmdstanr}
...
```{r results_func2_fullrank_plots, eval=use_cmdstanr}
plot_rank_hist(results_func2_fullrank)
plot_ecdf_diff(results_func2_fullrank)
```

This is not so surprising as we know that Stan's variational inference just isn't very good. See also the [ADVI vignette for plain Stan models](https://hyunjimoon.github.io/SBC/articles/computational_algorithm1.html).
