...
calculate_dap <- function(mu, var, generator, backened, fixed_args){
  ...
  for(i in 1:nsims){
    ...
    B <- B + mean(draws_eta)
    V <- V +  sum((draws_eta - mean(draws_eta))^2)
  }
  ...
  return(list(mu=mu, var=var, draws_eta=draws_eta, draws_Y=draws_Y, B=B, V=V))
}
```
...
gridsize_mu <- 5
gridsize_var <- 5
mu_seq <- seq(0, 1, length.out = gridsize_mu)
...
colnames(B_ghat_df)[colnames(B_ghat_df) == "Freq"] <- "B_hat"
...
colnames(V_ghat_df)[colnames(V_ghat_df) == "Freq"] <- "V_hat"
