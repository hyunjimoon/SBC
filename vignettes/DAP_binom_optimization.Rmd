...
calculate_dap <- function(mu, var, generator, backened, fixed_args){
  ...
  for(i in 1:nsims){
    ...
    B <- B + mean(draws_eta) - prior_thetas[i]
    V_2 <- V_2 + sum((draws_eta - mean(draws_eta))^2)
    V_3 <- V_3 + (mean(draws_eta) - prior_thetas[i])
  }
  
  var_theta_tilde_bar <- var_theta_tilde_bar / fixed_args$nsims
  var_theta_tilde <- var_theta_tilde / (fixed_args$nsims * ndraws)
  
  B <- B / fixed_args$nsims
  V_2 <- V_2 / (fixed_args$nsims * ndraws)
  V_3 <- (V_3 / fixed_args$nsims)^2
  ...
  return(list(mu=mu, var=var, draws_eta=draws_eta, draws_Y=draws_Y, var_theta_tilde_bar=var_theta_tilde_bar, var_theta_tilde=var_theta_tilde, theta_bar=theta_bar, B=B, V=var_theta_tilde_bar+V_2-V_3))
}
```
...
gridsize_mu <- 10
gridsize_var <- 10
mu_seq <- seq(-5, 5, length.out = gridsize_mu)
var_seq <- seq(1, 100, length.out = gridsize_var)
...
B <- array(rep(NA, grid_size), dim = c(gridsize_mu, gridsize_var))
V <- array(rep(NA, grid_size), dim = c(gridsize_mu, gridsize_var))
rownames(B) <- mu_seq
rownames(V) <- mu_seq
colnames(B) <- var_seq
colnames(V) <- var_seq
...
for(j in 1:length(var_seq)){
  for(i in 1:length(mu_seq)){
    ...
    B[i, j] <- dap$B
    V[i, j] <- dap$V
    dap_hmc <- calculate_dap(mu_seq[[i]],var_seq[[j]] , calib_generator, backend_binom_hmc, fixed_args_binom)
    B_g[i, j] <- dap_hmc$B
    V_g[i, j] <- dap_hmc$V
  }
}
...
colnames(B_df)[colnames(B_df) == "Freq"] <- "B_ghat"
...
ggplot(B_df) + geom_point(aes(x=Var, y=B_ghat, color="red")) + geom_line(aes(x=Var, y=B_ghat, color="red", group=1)) + ggtitle("B(red=ghat)") + geom_point(aes(x=Var, y=B_g, color="green")) + geom_line(aes(x=Var, y=B_g, group=2, color="green")) + theme(legend.position="none")
...
colnames(V_df)[colnames(V_df) == "Freq"] <- "V_ghat"
...
ggplot(V_df) + geom_point(aes(x=Var, y=V_ghat, color="red")) + geom_line(aes(x=Var, y=V_ghat, color="red", group=1)) + ggtitle("V(red=ghat)") + geom_point(aes(x=Var, y=V_g, color="green")) + geom_line(aes(x=Var, y=V_g, group=2, color="green")) + theme(legend.position="none")
