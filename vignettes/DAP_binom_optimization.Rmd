...
B_ghat_df[, "sd"] <- sqrt(as.numeric(as.vector(B_ghat_df$Var)))
...
result_good <- filter(B_ghat_df, Mu == mu_good & Var == var_good)
result_bad <- filter(B_ghat_df, Mu == mu_bad & Var == var_bad)

B_g_good <- result_good$B_g
B_ghat_good <- result_good$B_ghat

B_g_bad <- result_bad$B_g
B_ghat_bad <- result_bad$B_ghat

lambda_axis <- c(0, 1)
g_axis <- c(0, 1)
z_arr <- array(c(B_g_good, B_ghat_good, B_g_bad, B_ghat_bad), dim=c(2, 2))

persp3D(x=lambda_axis, y=g_axis, z = z_arr, theta=55, phi=10, xlab="lambda", ylab="g", zlab="B", ticks=5, ticktype="detailed")
