...
fig.dir <- here::here("vignettes/_figures_SBC_cache")
...
p <- plot_rank_hist(res_1000)
p
ggsave(file.path(fig.dir, "sbc_rank_hist_1000.pdf"), p, device = grDevices::pdf)
```
...
```{r ecdf_1000}
p <- plot_ecdf(res_1000) + theme(legend.position = "bottom")
p
ggsave(file.path(fig.dir, "sbc_ecdf_1000.pdf"), p)
```
...
```{r ecdf_diff_1000}
p <- plot_ecdf_diff(res_1000) + theme(legend.position = "bottom")
p
ggsave(file.path(fig.dir, "sbc_ecdf_diff_1000.pdf"), p)
```
...
```{r coverage_50}
p <- plot_coverage(res_50) 
p
ggsave(file.path(fig.dir, "sbc_coverage_50.pdf"), p)
```
...
```{r coverage_diff_50}
p <- plot_coverage_diff(res_50)
p
ggsave(file.path(fig.dir, "sbc_coverage_diff_50.pdf"), p)
```
