...
Now some fits still produce problematic Rhats or divergent transitions, let's brows the full statistics to 
...
empirical_coverage(stats_subset, c(0.25,0.5,0.9,0.95))
```
This actually shows a limitation of the coverage results - for `mu[2]` the approximate CI for coverage excludes exact calibration for a bunch of intervals, but above we see that the more trustworthy `plot_ecdf_diff` is not showing a problem (although there is some tendency).

This might need further investigation if discrepancies in `mu` are considered important, if we are interested only in the `beta` coefficients, we are assured that their calibration is pretty good.
