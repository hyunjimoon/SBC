...
We can even understand the reason for the positive correlation - it is because mean of our response beta distribution is `exp(linpred[1,]) / ( exp(linpred[1,]) + exp(linpred[2,]))`,
so increasing both linear predictors at the same time results in the same mean (but different variance). Since mean is usually more constrained than variance of the response, we get this
ridge.
...
```{r}
stats_beta1 <- results_combined_reject_more$stats[results_combined_reject_more$stats$parameter == "beta[1]",]
