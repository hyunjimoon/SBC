...
As you can see, the `Generator` returns a named list containing random samples from the prior and generated data realized from the prior samples.
The `$generated` member will be passed as-is to our sampling algorithm, so here we have a format that Stan will accept, but when using models from other
packages, the format needs to conform to what the packages expect.
...
if(use_cmdstanr) {
  ...
  poisson_backend <- rstan_sample_SBC_backend(
      rstan_model, iter = 2000, warmup = 1000)
}
```
