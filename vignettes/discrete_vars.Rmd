...
We can now write the same model in JAGS:
...
```{r}
results_jags <- compute_SBC(bind_datasets(datasets_2, datasets_3), backend_jags,
                        cache_mode = "results",
...
```{r}
library(tidyverse)
res_jags$stats %>% filter(variable != "s") %>%
  ggplot(aes(x = ess_tail)) + geom_histogram() + facet_wrap(~variable)
```

```{r}
results_all$stats %>% filter(variable != "s") %>%
  ggplot(aes(x = ess_tail)) + geom_histogram() + facet_wrap(~variable)
```
