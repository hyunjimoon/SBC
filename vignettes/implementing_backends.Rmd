...
practice for package development and will be necessary for paralellization later
(there are is some extra work needed for parallel processing to work, see notes below).
...
comfortable to use. Let's walk through the options and see how they can be implemented for the 
`glm` wrapper.
...
separation"). This problem is however only signaled as a warning and never 
...
diagnostic_types.SBC_glm_diagnostics <- function(x) {
  list(
    probs_0_1 = SBC_logical_diagnostic("had 0/1 probabilities", error_value = TRUE),
    converged = SBC_logical_diagnostic("had convergence problems", error_value = FALSE)
  )
}
```
...
The coverages are close to nominal:

```{r indo_sim_coverage}
plot_coverage_diff(res_indo_simple)
```
