...
#prior_dap_gr <- list(mu_1 = c(), mu_2 = c(),  dap_mu_1 = c(), dap_mu_2 = c(), dap_middle = c())
# prior_dap_gr <- list()
...
# Run self-calibration for gamma-hmc model
...
lambda_init_gamma <- list(
  shape = list(alpha=0.01, beta=0.01),
  a = list(mu=2, sigma=5)
)
...
# after calibration
datasets_gr_new <- generate_datasets(SBC_generator_function(generator_gr, sc_opt_gr$alpha, sc_opt_gr$beta, fixed_args_gamma), n_datasets = fixed_args_gamma$nsims)
result_gr_hmc_new <- compute_results(datasets_gr_new, backend_gr_hmc)
    
plot_rank_hist(result_gr_hmc_new)
ggplot(sc_opt_gr$t_df) + geom_point(aes(x=iter, y=lambda_loss))
```
