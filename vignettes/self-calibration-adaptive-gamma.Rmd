...
### 1. Condition calibrated region for one param:
plot l (x-T(x))^2 for shape mu  vs intercept mu
p1) fix intercept as  (2,1)
p2) fix intercept as (2,5)

```{R, warning = FALSE, error = FALSE}
calculate_dap <- function(mu, sigma, generator, backened, fixed_args){
  lambda_init_gamma <- list(
    shape = list(alpha= mu^2 / sigma^2, beta= mu / sigma^2),
    a = list(mu=2, sigma=1)
  )
...
for (mu in seq(1, 5, length.out = 4)){
  for(var in seq(1, 5, length.out = 4)){
    ...
    dap <- calculate_dap(mu, sqrt(var), calib_generator, backend_gr_opt, fixed_args_gr)
...
ggplot(prior_dap)+  geom_point(aes(x=mu, y=sigma), color = "red") + geom_point(aes(x=dap_mu, y = dap_sigma), color = "blue")
...
calculate_dap <- function(shape_mu, a_mu, generator, backened, fixed_args){
  ...
  for(fit in sbc_result$fits){
    samples <- SBC_fit_to_draws_matrix(fit)
    draws_eta <- c(draws_eta, posterior::extract_variable(samples, "shape")) ### TODO
  }
