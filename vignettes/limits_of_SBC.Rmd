...
No data, should be correlated but is ucorrelated
...
gq_corr <- generated_quantities(sum_a_b = a + b, 
                                diff_a_b = a - b,
                                prod_a_b = a * b,
                                mvn_log_lik = mvtnorm::dmvnorm(c(a, b), sigma = mvn_sigma, log = TRUE))

res_corr <- compute_SBC(datasets_sigma, backend_corr, keep_fits = FALSE,
                        gen_quants = gq_corr, globals = "mvn_sigma")
```
...
gq_corr <- generated_quantities(sum = mu[1] + mu[2], 
                                diff = mu[1] - mu[2],
                                prod = mu[1] * mu[2],
                                log_lik = sum(dnorm(y[,1], mean = mu[1], sd = 1, log = TRUE) + dnorm(y[,2], mean = mu[2], sd = 1, log = TRUE)))

res_corr <- compute_SBC(datasets_correlated, backend_uncorr, keep_fits = FALSE,
                        gen_quants = gq_corr, globals = c("SBC_fit.analytic_backend_uncorr",
