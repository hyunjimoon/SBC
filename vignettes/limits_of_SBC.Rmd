...
Let us use this model for the same set of simulations. To see the sensitivity of different generated quantities, we additionally, add a generated quantity representing the likelihood of just the datapoints ignored by the model and then of just the last datapoint (which is one of the ignored).

```{r}
log_lik_gq_2 <- bind_generated_quantities(
  log_lik_gq,
  generated_quantities(log_lik_ignored = normal_lpdf(y[ceiling(N/2):N], mu, 1),
                       log_lik_last = normal_lpdf(y[N], mu, 1)))
results_missing_2 <- compute_SBC(datasets_missing, backend_missing_2, gen_quants = log_lik_gq_2, 
                    cache_mode = "results", 
...
Now contraction is pretty high, and `mu` is behaving well, but our `log_lik` generated quantity shows a clear problem. Further, looking at just the actually ignored datapoints results in a stronger violation and would likely be detected earlier, while just looking at a single ignored datapoint also violates uniformity, indicating that inspecting per-datapoint likelihoods might be reasonable in some cases.
