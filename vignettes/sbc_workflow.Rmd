...
mixture_first_pairs <- mcmc_pairs(results_first$fits[[1]]$draws(), np = nuts_params(results_first$fits[[1]]))
mixture_first_pairs
ggsave(file.path(cache_dir, "sbcworkflow_mixture_first_pairs.pdf"), plot = mixture_first_pairs)
```
...
ggsave(file.path(cache_dir, "sbcworkflow_mixture_fixed_log_mix_pairs.pdf"), mixture_fixed_log_mix_pairs)
```
...
mixture_fixed_ordered_pairs <- mcmc_pairs(problematic_fit$draws(), np = nuts_params(problematic_fit), max_treedepth = 10)
mixture_fixed_ordered_pairs
ggsave(file.path(cache_dir, "sbcworkflow_mixture_fixed_ordered_pairs.pdf"), mixture_fixed_ordered_pairs)
```
...
ggsave(file.path(cache_dir, "sbcworkflow_results_fixed_ordered_combined_results.pdf"), combined)
```
...
backend_logistic_first <- SBC_backend_cmdstan_sample(model_logistic_first, chains = 2) 
```
...
```{r}
results_logistic_first_10 <- compute_results(datasets_logistic_first, backend_logistic_first, 
                    cache_mode = "results", 
...
ggsave(file.path(cache_dir, "sbcworkflow_logistic_first_results.pdf"), combined)
```
...
ggsave(file.path(cache_dir, "sbcworkflow_logistic_first_sim_estimated.pdf"), logistic_first_sim_estimated)
```

```{r}
#datasets_logistic_first$generated[[1]]$X %*% datasets_logistic_first$generated[[1]]$beta

logistic_loglik_gq <- generated_quantities(log_lik = sum(dbinom(y, size = 1, prob = plogis(alpha + X %*% beta), log = TRUE)))
results_logistic_first_10_gq <- 
  recompute_statistics(results_logistic_first_10, datasets_logistic_first,
                       backend_logistic_first,
                       gen_quants = logistic_loglik_gq)
```

```{r logistic_first_gq_results}
plot_rank_hist(results_logistic_first_10_gq)
plot_ecdf_diff(results_logistic_first_10_gq)
```

```{r}
results_gq_loglik_only <- plot_rank_hist(results_logistic_first_10_gq, parameters = "log_lik") | plot_ecdf_diff(results_logistic_first_10_gq, parameters = "log_lik")

ggsave(file.path(cache_dir, "sbcworkflow_results_gq_loglik_only.pdf"), results_gq_loglik_only, width = 6, height = 1.8)
...
```{r}
model_logistic_matrix <- cmdstan_model("small_model_workflow/logistic_matrix.stan")
backend_logistic_matrix <- SBC_backend_cmdstan_sample(model_logistic_matrix, chains = 2) 
```

```{r}
generator_func_logistic_matrix <- function(N_obs, N_predictors) {
...
generator_logistic_matrix <- SBC_generator_function(generator_func_logistic_matrix, N_obs = 50, N_predictors = 3)
```

```{r}
set.seed(125488)
datasets_logistic_matrix_10 <- generate_datasets(generator_logistic_matrix, 10)
```



```{r}
logistic_matrix_loglik_gq <- generated_quantities(log_lik = sum(dbinom(y, size = 1, prob = plogis(X %*% beta), log = TRUE)))


results_logistic_matrix_10 <- compute_results(
  datasets_logistic_matrix_10, backend_logistic_matrix,
  gen_quants = logistic_matrix_loglik_gq,
  cache_mode = "results",
  cache_location = file.path(cache_dir, "logistic_matrix_10"))
```

```{r logistic_matrix_results}
logistic_matrix_ranks <-  plot_rank_hist(results_logistic_matrix_10)
logistic_matrix_ranks
logistic_matrix_ecdf <- plot_ecdf_diff(results_logistic_matrix_10)
logistic_matrix_ecdf

combined <- logistic_matrix_ranks | logistic_matrix_ecdf
ggsave(file.path(cache_dir, "sbcworkflow_logistic_matrix_results.pdf"), combined)
```
...
```{r}
results_logistic_fixed_prior_10 <- compute_results(
  datasets_logistic_matrix_10, backend_logistic_fixed_prior,
  gen_quants = logistic_matrix_loglik_gq,
  cache_mode = "results",
...
```{r}
set.seed(32464655)
datasets_logistic_matrix_200 <- generate_datasets(generator_logistic_matrix, 200)

results_logistic_fixed_prior_200 <- compute_results(
  datasets_logistic_matrix_200, backend_logistic_fixed_prior,
  keep_fits = FALSE,
  gen_quants = logistic_matrix_loglik_gq,
  cache_mode = "results",
...
fanos_plot <-  ggplot(fanos_df, aes(x = fano)) + 
  ...
  geom_vline(xintercept = 1.5, color = "blue", size = 2) +
...
combined_reject_coverage <- plot_coverage(results_combined_reject_more)
...
that the posterior 90% interval for `beta[2]` excludes zero, i.e. that we learn something about the sign of the association
with a continuous predictor: 

```{r}
stats_beta2 <- 
  results_combined_reject_more$stats[
    results_combined_reject_more$stats$parameter == "beta[2]",]
