...
```{r}
generator_func_ordered_reject <- function(N) {
  repeat {
    mu <- sort(rnorm(2, 3, 1)) 
    if(abs(mu[2] - mu[1]) > 0.5) {
      break
    }
  }
  theta <- runif(1)
...
warning("Undo this")
generator_ordered_reject <- generator_ordered
#generator_ordered_reject <- function_SBC_generator(generator_func_ordered_reject, N = 50)
```


```{r}
set.seed(3785432)
datasets_ordered_reject_10 <- generate_datasets(generator_ordered_reject, 10)
```

```{r}
results_ordered_reject_10 <- compute_results(datasets_ordered_reject_10, backend_fixed_ordered)
```

```{r}
max(results_ordered_reject_10$stats$rhat)
min(results_ordered_reject_10$stats$ess_bulk)
min(results_ordered_reject_10$stats$ess_tail)
plot_ecdf_diff(results_ordered_reject_10)
```
```{r}
set.seed(21368755)
datasets_ordered_reject_90 <- generate_datasets(generator_ordered_reject, 90) 
results_ordered_reject_next <- compute_results(datasets_ordered_reject_90, backend_fixed_ordered)
```

```{r}
library(tidyverse)
results_ordered_reject_100$stats %>% arrange()
```
...
results_ordered_reject_100 <- bind_results(results_ordered_reject_10, results_ordered_reject_next)

max(results_ordered_reject_100$stats$rhat)
min(results_ordered_reject_100$stats$ess_bulk)
min(results_ordered_reject_100$stats$ess_tail)


plot_ecdf_diff(results_ordered_reject_100)
```
