...
for(i in 1:length(mu_seq)){
  for(j in 1:length(var_seq)){
    ...
    lambda_diff[i,j] <- (mu_seq[[i]] - dap$mu) + (var_seq[[j]] - dap$var)
...
calc_mse <- function(p){
  return(sum(p * as.vector(squared_lambda_diff)))
}
calc_mse_grad  <- function(p){
  return(as.vector(squared_lambda_diff))
}

calc_mse2 <- function(p2){
  t_lambda <- c(as.vector(dap_lambda_mu), as.vector(dap_lambda_var))
  lambda <- c(as.vector(lambda_mu), as.vector(lambda_var))
  diff <- t_lambda - lambda
  sum(p2 * diff)^2 + sum(p2 * (t_lambda - sum(p2 * t_lambda))^2)
}

calc_mse2_grad <- function(p2){
  t_lambda <- c(as.vector(dap_lambda_mu), as.vector(dap_lambda_var))
  lambda <- c(as.vector(lambda_mu), as.vector(lambda_var))
  diff <- t_lambda - lambda
  2 * sum(p2 * diff) * diff + 2 * (t_lambda - sum(p2 * t_lambda)) * -((p2^2 * t_lambda)/2)
}
...
local_opts <- list(algorithm="NLOPT_LD_LBFGS", xtol_rel = 1.0e-8)
penalty_opts <- list(algorithm="NLOPT_LD_AUGLAG_EQ", print_level=1, xtol_rel=1e-8, maxeval=20, check_derivatives=TRUE, local_opts=local_opts)
mse_opts2 <- nloptr::nloptr(rep(rep(1 / length(p_init), length(p_init)), 2),
                               calc_mse2,
                               eval_grad_f = calc_mse2_grad,
                               eval_g_eq = mse2_eq_constraints,
                               opts = penalty_opts,
                               lb=rep(prob_lb, 2), ub=rep(prob_ub, 2))
