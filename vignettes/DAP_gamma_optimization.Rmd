...
calc_mse2 <- function(p){
  n <- length(p)
  dap_lambda_mu_vec <- as.vector(dap_lambda_mu)
  dap_lambda_var_vec <- as.vector(dap_lambda_var)
  lambda_mu_vec <- as.vector(lambda_mu)
  lambda_var_vec <- as.vector(lambda_var)
  
  bias_squared_term <- mean(p * (dap_lambda_mu_vec - lambda_mu_vec))^2 + mean(p * (dap_lambda_var_vec - lambda_var_vec))^2
  
  variance_term <- (var(p * (dap_lambda_mu_vec - lambda_mu_vec)) + var(p * (dap_lambda_var_vec - lambda_var_vec))) * (n - 1)/n
...
calc_mse2_grad <- function(p){
  ...
  bias_square_grad_mu <- 2 * ( mean(p * (dap_lambda_mu_vec - lambda_mu_vec))) * (dap_lambda_mu_vec - lambda_mu_vec) / n_p
  bias_square_grad_var <- 2 * ( mean(p * (dap_lambda_var_vec - lambda_var_vec))) * (dap_lambda_var_vec - lambda_var_vec) / n_p
  
  
  #var_grad_mu <- 2/n_p * (p * dap_lambda_mu_vec - mean(p * dap_lambda_mu_vec)) * (dap_lambda_mu_vec - dap_lambda_mu_vec/n_p)
  
  var_grad_mu <- (2 * (p * dap_lambda_mu_vec - mean(p * dap_lambda_mu_vec)) * (dap_lambda_mu_vec - dap_lambda_mu_vec/n_p)) / n_p
  var_grad_lambda <- (2 * (p * dap_lambda_var_vec - mean(p * dap_lambda_var_vec)) * (dap_lambda_var_vec - dap_lambda_var_vec/n_p)) / n_p
