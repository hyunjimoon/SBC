...
calculate_dap <- function(lambda_mu, lambda_var, generator, datasets=NULL, backened, fixed_args){
  ...
  draws_params <- c()
  draws_Y <- c()
  prior_thetas <- posterior::extract_variable(datasets$variables, "log_tau")
...
gridsize_var <- 5
mu_seq <- c(-1, 0, 1, 2, 3)
var_seq <- c(0.01^2, 0.1^2, 1^2, 2^2, 3^2)
grid_size <- length(mu_seq) * length(var_seq)
...
dap_df[, "dap_var"] <- as.numeric(dap_lambda_df$Freq)
ggplot(dap_df, mapping=aes(x="mu", y="var")) + geom_point(aes(x=mu, y=var), color = "red") + geom_point(aes(x=dap_mu, y = dap_var), color = "blue") + scale_y_continuous(labels=scaleFUN) + geom_segment(aes(x=mu, y=var, xend=dap_mu, yend=dap_var), size=0.2, arrow = arrow(length = unit(0.3, "cm"))) + xlab("mean") + ylab("variance") + ggtitle("HMC")
```

Although to a lesser degree, we can still observe extreme deviations from the prior. Therefore, we rule out the possiblity of a sampling error but instead assume that undesirable prior regions are present.
...
ggplot(dap_df, mapping=aes(x="mu", y="var")) + geom_point(aes(x=mu, y=var), color = "red") + geom_point(aes(x=dap_mu, y = dap_var), color = "blue") + scale_y_continuous(labels=scaleFUN) + geom_segment(aes(x=mu, y=var, xend=dap_mu, yend=dap_var), size=0.2, arrow = arrow(length = unit(0.3, "cm"))) + xlab("mean") + ylab("variance") + ggtitle("HMC") + coord_cartesian(ylim=c(0, 7), xlim = c(-10, 5)) + scale_y_continuous(breaks=c(1:9))
```
We can see that variance plays the big factor; large variance values(>=1) start to show miscalibration.
...
ggplot(B_hmc_df) + geom_point(aes(x=coords, y=B_optim, color="red")) + ggtitle("B(red=optimization, green=HMC), closer to 0 is better") + geom_point(aes(x=coords, y=B_hmc, color="green")) + geom_segment(aes(x=coords, y=B_optim, xend=coords, yend=B_hmc), size=0.2) + theme(axis.title.x = element_text(margin=unit(c(10, 0, 0, 0), "mm"))) + xlab("meam, variance") + ylab("B") + theme(legend.position = "none") + geom_hline(yintercept = 0, linetype="dashed", size=1)
```

The X axis represent the prior mean-variance combinations. The bias metric values are shown on the Y axis. We plotted the bias metric calculated from optimization as red dots, and green dots for HMC. Excluding extreme variance values, we can actually see that the two inference algorithms do not vary by much.

Let's also examine the bias values individually. Like from the above arrow plots, we can see that variance is the big factor when it comes to calibration. Variance values smaller than 0.1 have fantastic bias metrics, but as variance gets larger, the bias metrics also get big, telling us that the DAP gets further and further away. 

In addition, the mean hyperparameter does not show any significant influence apart from extreme variance values, which would already be disregarded when actually comparing models.

Overall, we can conclude that priors with variance closer to 0 indicate better calibration, in the context of the bias metric.
...
But for the variance metric, we can see that for larger variance prior values, HMC outperforms optimization by a big margin, suggesting HMC under/over-estimates to a lesser degree when priors become hard to perform inference. Otherwise, the plot strengthens our belief that the variance parameter is the most influential, and that smaller values are preferable.


### Iteratively updating prior for well-calibration

Up to now, we've worked with metrics and identified miscalibrated regions. Can we try and go the other direction? How can we use the prior-posterior discrepancy information to refine our prior? In this section, we explore a prior update algorithm which attempts to move from a badly calibrated region to a more well-calibrated area.
...
sc_eightschools_opt <- self_calib_adaptive(calib_generator, backend_eightschools_opt, updator, c("log_tau"), lambda_init_eightschools, nsims, 15, gamma, tol, fixed_args = fixed_args_eightschools)
```
...
Unfortunately, no apparent improvement could be identified. The prior on $\mathrm{log}(\tau)$ was found to be extremely sensitive to subtle changes to the variance. Therefore, the iterative algorithm is assumed to have difficulties in fine-tuning the hyperparameters.
...
sc_eightschools_opt <- self_calib_adaptive(calib_generator, backend_eightschools_hmc, updator, c("log_tau"), lambda_init_eightschools, nsims, 10, gamma, tol, fixed_args = fixed_args_eightschools)
...
This proved to be only marginally better but the algorithm still struggled to find better calibrated priors even after 10 iterations.
