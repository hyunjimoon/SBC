...
Like the image above, we can use graphical plots to get a rough idea of the pathological details; under vs overdispersion, under vs overestimation, and so on. But it's not trivial to *quantify* how much the the inference algorithm is misbehaving - in fact, there isn't a generally applicable way to automatically identify the verdict of SBC; it relies on the user to identify patterns from the plots themselves.

---

We propose a different test method, based on two concepts, the Data Averaged Posterior and the self-consistency metric. Our motivations is that by taking advantage of the multiple "generate and fit" iterations of SBC, it will give us enough information to construct a more interpretable metric of miscalibration.
...
$$
B(\hat{g}, \lambda) := E[\hat{\theta} \ | \ \theta_{1,...,M}] - \bar{\theta} = \frac{1}{M} \sum_M (\hat{\theta_m} - \theta_m)
$$
...
Finally, we'll try to start from a "bad", miscalibrated prior region and try to iteratively refine the prior into a more favorable region. We'll be starting from $N(5, 10)$ with optimization. Let's check the rank plots to see how bad the region is:
...
Looks much better than the starting location.
...
Finally, we showed by iteratively updating the prior we were able to go from a poorly calibrated region to a well-calibrated region.
